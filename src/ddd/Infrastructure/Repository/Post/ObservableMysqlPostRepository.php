...
class ObservableMysqlPostRepository implements PostRepository
{
    ...
    {
        $promiseOfQuery = $this->mysqlClient->query('SELECT p.*, a.type as author_type, (SELECT COUNT(id) FROM comment c WHERE c.post_id = p.id) as comment_count FROM post p LEFT JOIN author a ON p.author_id = a.id ORDER BY p.datePublished DESC');
    ...
    public function findById(PostId $postId): PromiseInterface //of Post or Null
    {
        $promiseOfQuery = $this->mysqlClient->query(
            "SELECT p.*, (SELECT COUNT(id) FROM comment c WHERE c.post_id = p.id) as comment_count FROM post p where p.id = ?",
            [$postId->value]
        );
    ...
    {
        $promiseOfQuery = $this->mysqlClient->query(
            "SELECT p.*, (SELECT COUNT(id) FROM comment c WHERE c.post_id = p.id) as comment_count FROM post p WHERE p.slug = ?",
            [(string) $slug]
        );
    ...
    {
        $promiseOfQuery = $this->mysqlClient->query(
            "SELECT p.*, a.type as author_type, (SELECT COUNT(id) FROM comment c WHERE c.post_id = p.id) as comment_count FROM post p LEFT JOIN author a ON p.author_id = a.id WHERE LOWER(p.author) = LOWER(?) AND p.slug = ?",
